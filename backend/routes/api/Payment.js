...
async function TransferToRecipient(req, res, courseId, amount) {
  await User.findOne({ "teacherCourse.courseId": courseId })
    .then(async data => {
      console.log(amount);
      await omise.transfers.create(
        { amount: amount, recipient: data.rpid },
...
router.post(
  ...
  (req, res) => {
    User.findOne({ "teacherCourse.courseId": req.body.courseId })
      .then(data => {
...
router.post(
  ...
  (req, res) => {
    ...
    User.findOneAndUpdate(
      ...
      {
        teacherPayment_AccountHolderName:
          req.body.teacherPayment_AccountHolderName,
...
async function UpdateOmiseRecipient(
  req,
  res,
  name,
  email,
